...
const SearchMovie = ({ menuSwitch, setMenuSwitch }) => {
    ...
    const getSearchVideo = async () => {
        try {
            const res = await axios.get(`${search_url}${pageToken}&q=${type}&key=${process.env.REACT_APP_API_KEY}`);
    ...
    return (
        <> 
            <div className={`fixed top-[33%] left-[40%] rounded-xl bg-gray-700 p-4 w-96 z-70 ${filter ? 'block' : 'hidden'}`}>
