...
const LeftSection = ({ menuSwitch, setMenuSwitch }) => {
  ...
  return (
    <>
      <div
        className={`z-50 md:z-0 ${
          menuSwitch ? "w-3/5 md:w-60" : "w-0 md:w-20"
        } h-screen ${
          menuSwitch ? "p-4" : "p-0"
        } overflow-x-hidden fixed md:sticky top-16 left-0 scrollbar-thin scrollbar-thumb-rounded-md hover:scrollbar-thumb-gray-300 scrollbar-track-transparent bg-background-black`}
      >
        {menuSwitch ? (
          <div>
              ...
              return (
                <Link to={item.link ? item.link : ""}>
                  <div
                    ...
                    } pr-4 pl-[6px] py-2 rounded-xl cursor-pointer `}
            ...
            {sectionIcons_2_batch.map((item) => {
              return (
                <div
                  key={item.id}
                  className="flex justify-start items-center hover:bg-gray-50 hover:bg-opacity-5 pr-4 pl-[6px] py-2 rounded-xl cursor-pointer  "
                  onClick={() => setMenuSwitch(false)}
                >
            ...
            <p className="text-gray-400 my-4 text-sm px-4  ">
              Terms Privacy Policy & Safety How youTube works Test new features
            </p>
            <h6 className="text-gray-500 text-xs px-4  ">
          ...
          sectionIcons_4_batch.map((item) => {
            return (
              <div
                key={item.id}
                className="flex flex-col justify-center items-center py-3 px-2 rounded-md hover:bg-gray-50 hover:bg-opacity-5 my-2 cursor-pointer  "
                onClick={() => setMenuSwitch(false)}
              >
