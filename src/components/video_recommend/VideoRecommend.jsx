...
const VideoRecommend = ({ id }) => {
  ...
  const getRelatedVideos = async () => {
    try {
      const res = await axios.get(`${feed_url}${pageToken}&key=${process.env.REACT_APP_API_KEY}`);
      ...
      >

        {
          videoRecommend.map((item) => (
            <>
              <div className='hidden md:grid grid-cols-2 items-center justify-start w-full mb-1' key={item?.id}>
                <Link to={`/watch/${item?.id}`}>  <div className={`relative h-[5.5rem] w-[85%] mx-auto bg-gray-50 rounded-2xl bg-center mb-2 cursor-pointer`} >
                  <img src={item?.snippet?.thumbnails?.high?.url} alt={item?.snippet?.localized?.title} className='w-full h-full rounded-lg object-cover' />
                </div>
                </Link>
                <div className="flex items-start flex-col h-full">
                  <Link to={`/watch/${item?.id}`}>
                    <h3 className='text-white font-medium line-clamp-2 h-11 text-sm cursor-pointer'>{item?.snippet?.localized?.title}</h3>
                  </Link>
                  <h4 className='text-gray-300 text-xs hover:text-white pb-1 cursor-pointer'>{item?.snippet?.channelTitle}</h4>
                  <span className='flex text-gray-300 text-xs'>
                    ...
                    {<Time time={moment(new Date(item?.snippet?.publishedAt)).fromNow()}/>}
                  </span>
                </div>
              </div>
